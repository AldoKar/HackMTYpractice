...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Calculate if all cards are stacked
    const lastCard = cardsRef.current[cardsRef.current.length - 1];
    const lastCardTop = lastCard ? getElementOffset(lastCard) : 0;
    const lastCardTriggerStart = lastCardTop - stackPositionPx - itemStackDistance * (cardsRef.current.length - 1);
    const allCardsStacked = scrollTop >= lastCardTriggerStart;

    // Extend pinEnd if not all cards are stacked yet
    const effectivePinEnd = allCardsStacked ? endElementTop - containerHeight / 2 : lastCardTriggerStart + containerHeight * 2;
